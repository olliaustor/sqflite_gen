    ...
    () => {
      group(
        ...
        () => {
          ...
          test('returns string with all lower case expect first character', () {
            final result =
      ...
      group(
        ...
        () => {
          ...
          test('changes all word to start with lowercase', () {
            final result =
                'This IS a Test t0 test 8 random WOrds'.toStartWithLowerCase();
            expect(
              result,
              equals('this IS a Test t0 test 8 random WOrds'),
            );
      ...
      group(
        ...
        () => {
          ...
          test('call with empty list returns false', () {
            final result =
                'This IS a Test t0 test 8 random WOrds'.containsAny([]);
            expect(result, equals(false));
          }),
          test('call with not contained value returns false', () {
            final result =
                'This IS a Test t0 test 8 random WOrds'.containsAny(['blubb']);
            expect(result, equals(false));
          }),
          test('call with not contained value returns false', () {
            final result = 'This IS a Test t0 test 8 random WOrds'.containsAny([
              'blubb',
              'Test',
            ]);
      ...
      group(
        ...
        () => {
          ...
          test('string with empty replacement list returns same string', () {
            final map = <MapEntry<String, String>>[];
            final result = 'test'.replaceAllFromList(map);
            expect(result, equals('test'));
          }),
          test('string with different replacement list returns same string',
              () {
          ...
          test('replacement list replaces all occurrences', () {
            final map = [const MapEntry('test', 'value')];
            final result = 'test test blubb'.replaceAllFromList(map);
